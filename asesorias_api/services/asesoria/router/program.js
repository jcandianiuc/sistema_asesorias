...
const { CreateProgram, ViewProgram, FetchPrograms } = require(`${basePath}/src/application/program`);
const { ProgramaEducativoRepository } = require(`${basePath}/src/infrastructure/repositories/typeorm`);
...
router.get('/', async (ctx, next) => {
  ...
  try {
    const repository = new ProgramaEducativoRepository(db);
    const service = new FetchPrograms(repository);
...
router.get('/:id', async (ctx, next) => {
  ...
  try {
    const repository = new ProgramaEducativoRepository(db);
    const service = new ViewProgram(repository);
...
router.post('/', koaBody(), async function (ctx, next) {
  ...
  try {
    ...
    const repository = new ProgramaEducativoRepository(db);
    const service = new CreateProgram(repository);
